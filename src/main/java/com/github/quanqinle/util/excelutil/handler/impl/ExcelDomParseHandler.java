...
public class ExcelDomParseHandler<T> extends BaseExcelParseHandler<T> {
	...
	private void checkHeader(Iterator<Row> rowIterator, ParserParam parserParam) {
		while (true) {
	...
	private Workbook generateWorkBook(ParserParam parserParam) throws IOException, InvalidFormatException {
		return WorkbookFactory.create(parserParam.getExcelInputStream());
	}

	private List<T> parseRowToTargetList(Iterator<Row> rowIterator, ParserParam parserParam)
